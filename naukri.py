...
mob = os.getenv("PHONE")  # Type your mobile number here
...
def LoadNaukri(headless):
    ...
    if headless:
        ...
        print('UserAgent Header: ', customUserAgent)
...
def UpdateProfile(driver):
    try:
        ...
        # close_locator = "//*[contains(@data-ga-track, 'EditProfile|BasicDetails|Cancel')]//span"
        ...
        if WaitTillElementPresent(driver, close_locator, "XPATH", 20):
...
def UpdateResume():
    try:
        ...
        for pageNum in range(pagecount - 1):
            output.addPage(existing_pdf.get_page(pageNum))

        page = existing_pdf.get_page(pagecount - 1)
...
def main():
    ...
    parser.add_argument("--headless", help="Script runs in ``headless`` mode without visible GUI", action="store_true")
